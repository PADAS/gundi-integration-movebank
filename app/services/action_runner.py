...
async def _execute_action_impl(
    ...
    try:  # Execute the action handler with a timeout
        ...
        if ephemeral_run.get():
            ...
            # waiting on an answer) and its synthetic integration has no next
            # tick, so the WARNING path below would read as success. Keep the
            # template's 504.
            return await _handle_error(
        ...
        # unhealthy.
        # Scoped to automated runs on a saved integration. A manual run has no
        # next tick — an operator clicked "Run now" and is waiting for an
        # answer, so a WARNING plus an HTTP 200 would read as success. Same
        # split the runner already applies to misconfigured pull actions via
        # skippable_pull. An ephemeral run is interactive for the same reason,
        # and its synthetic integration id must not receive activity logs.
        classified = classify_error(e)
        if (
            classified is not None
            and classified.error_type in RECOVERABLE_ERROR_TYPES
            and not is_manual
            and not ephemeral_run.get()
        ):
