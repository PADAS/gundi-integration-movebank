...
def test_redact_url_strips_credentials_and_brackets_ipv6():
    # userinfo, query and path are dropped (they can carry secrets); host[:port] kept.
    assert _redact_url("https://user:secret@example.com/path?token=abc") == "example.com"
    assert _redact_url("https://example.com:8080/p") == "example.com:8080"
    # IPv6 literal + port is bracketed so host:port stays unambiguous.
    assert _redact_url("http://[::1]:8080/p") == "[::1]:8080"
    assert _redact_url("http://[2001:db8::1]:443/x") == "[2001:db8::1]:443"
    # A non-numeric port must NOT collapse the whole URL to "<unparseable url>";
    # we still surface the safe host.
    assert _redact_url("https://example.com:abc/p") == "example.com"
    # An explicit port 0 (falsy but valid) is preserved.
    assert _redact_url("https://example.com:0/p") == "example.com:0"
